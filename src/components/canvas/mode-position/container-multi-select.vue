...
export default {
  ...
  computed: {
    layoutPosition () {
      const { layers, layoutLayers } = this.iocEditor
    ...
    visible () {
      return this.iocEditor.layoutLayers.length > 1
    }
  },
  methods: {
    handleWindowKeydown (event) {
      this.iocEditor.setKeyCode(event.keyCode)
    },
    handleWindowKeyUp () {
      this.iocEditor.setKeyCode()
    },
    handleClick () {
      this.visible && this.iocEditor.updateLayoutToLayers()
    },
    handleCancel () {
      this.iocEditor.setLayout()
    },
    handleAlign (event, align) {
      event.stopPropagation()
      const { layoutLayers, layoutLayersInfo } = this.iocEditor
      this.iocEditor.setLayoutLayersInfo(
