<template>
  <div
    ...
    :class="{'active': isActive, 'multi-select': isMultiSelect}"
  >
    <div
      :key="index"
      :class="point"
      class="container-operator__point"
    ...
    <div class="container-operator__header">
      <Tooltip
        :key="option.value"
        :title="option.label"
    ...
    <div class="container-operator__content">
      <slot></slot>
    </div>
  </div>
...
export default {
  ...
  inject: ['iocEditor', 'moveLayer', 'resizeLayer'],
  data () {
    return {
  ...
  computed: {
    isActive () {
      const { id } = this.layer
      const { active } = this.iocEditor
      return !!active && active.id === id
    },
    isMultiSelect () {
      const { id } = this.layer
      const { layoutLayers = [] } = this.iocEditor
      return layoutLayers.length > 1 && layoutLayers.includes(id)
    },
    style () {
      const { id, zIndex, position: { x, y }, size: { width, height }, fullscreen = false } = this.layer
      const { layoutLayers, layoutLayersInfo } = this.iocEditor
    ...
    menuOptions () {
      let options = []
      const length = this.iocEditor.layers.length
  ...
  methods: {
    handleClick (event) {
      ...
      this.iocEditor.clear()
      const { id } = this.layer
      if (this.iocEditor.keyCode === 91) {
        // 多选
        if (this.iocEditor.layoutLayers.includes(id)) {
          this.iocEditor.removeFromLayout(id)
        } else {
          this.iocEditor.addToLayout(id)
        }
      } else {
        this.iocEditor.setLayout([id])
      }
    },
    ...
    handleLevel (key) {
      const { id } = this.layer
      const levels = this.iocEditor.layers.map(({ id, zIndex }) => ({ id, zIndex })).sort((a, b) => a.zIndex - b.zIndex)
      ...
      switch (key) {
        ...
        case 'delete': {
          // 删除
          this.iocEditor.removeLayer(this.layer)
          break
        }
      }
      if (needUpdate) {
        ...
        // 重新这只图层层级
        this.iocEditor.setPrev()
        this.iocEditor.setLayersLevel(newLevel)
      }
    },
    handleDelete () {
      this.iocEditor.removeLayer(this.layer)
    ...
    handleMouseDownOnPoint (event, point) {
      ...
      const move = (moveEvent) => {
        ...
        switch (point) {
          case 't': {
            this.iocEditor.resizeLayer(id, { x, y: y + movementY }, { width, height: height - movementY })
            break
          }
          case 'r': {
            this.iocEditor.resizeLayer(id, { x, y }, { width: width + movementX, height })
            break
          }
          case 'b': {
            this.iocEditor.resizeLayer(id, { x, y }, { width, height: height + movementY })
            break
          }
          case 'l': {
            this.iocEditor.resizeLayer(id, { x: x + movementX, y }, { width: width - movementX, height })
            break
          }
          case 'lt': {
            this.iocEditor.resizeLayer(id, { x: x + movementX, y: y + movementY }, { width: width - movementX, height: height - movementY })
            break
          }
          case 'rt': {
            this.iocEditor.resizeLayer(id, { x, y: y + movementY }, { width: width + movementX, height: height - movementY })
            break
          }
          case 'lb': {
            this.iocEditor.resizeLayer(id, { x: x + movementX, y }, { width: width - movementX, height: height + movementY })
            break
          }
          case 'rb': {
            this.iocEditor.resizeLayer(id, { x, y }, { width: width + movementX, height: height + movementY })
            break
          }
        }
      }
...
.container-operator {
  ...
  &.active {
    .container-operator__header,
    .container-operator__point {
      display: flex;
    }
  }
  &.multi-select {
